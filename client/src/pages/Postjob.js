...
export default function Postjob() {
  ...
  const submit = (e) => {
    ...
    if (Object.keys(formErrors).length === 0 && isSubmit) {
      ...
      // Add proper address data
      formdata.append("address", `${address.house},${address.street},${address.town},${address.city},${address.district},${address.state},${address.pincode}`);
      formdata.append("city", address.city); // Add city separately if needed
      formdata.append("image", inputvalues.image);
      
      userAPI.postJob(formdata)
        .then((response) => {
          console.log(response);
